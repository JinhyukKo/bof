...
void makePerson(){
    ...
    if(type == ADULT){
        person->type.adult.job_idx = idx;
        person->type.adult.age = age;
    }else if(type == CHILD){
        person->type.child.job_idx = idx;
...
void transform(){
    ...
    }else if(person->typeflag == ADULT){
        if(person->type.adult.age <= 18){
            person->typeflag = CHILD;
        }
    }
    return;
}
...
void detail(){
    ...
    if(person->typeflag == ADULT){
        printf("Type: Adult\n");
        printf("Age: %d\n", person->type.adult.age);
        printf("Job: %s\n", job_names[person->type.adult.job_idx]);
    }else if(person->typeflag == CHILD){
        printf("Type: Child\n");
        printf("Age: %d\n", person->type.child.age);
        printf("Job: %s\n", job_names[person->type.child.job_idx]);
    }
    return;
}
...
void deletePerson(){
    ...
    if(person->typeflag == ADULT){
        free(job_names[person->type.adult.job_idx]);
        job_names[person->type.adult.job_idx] = NULL;
    }else if(person->typeflag == CHILD){
        free(job_names[person->type.child.job_idx]);
        job_names[person->type.adult.job_idx] = NULL;
    }
    free(person);
    town[idx] = NULL;
